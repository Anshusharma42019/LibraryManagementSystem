  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">{library?.name || 'Dashboard'}</h1>
        ...
        {daysLeft > 0 && daysLeft <= 15 && (
          <div className="bg-red-50 border border-red-200 rounded-xl px-3 py-2 flex items-center gap-2">
            ...
            <span className="text-xs text-red-600 font-medium">Subscription expires in {daysLeft} days</span>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard title="Total Students"    value={s.totalStudents ?? 0}    icon={Users}         color="text-indigo-600" bgColor="bg-indigo-50" subtitle="All time" />
        <StatCard title="Active Students"   value={s.activeStudents ?? 0}   icon={CheckCircle}   color="text-green-600"  bgColor="bg-green-50"  subtitle={`${s.expiringStudents ?? 0} expiring`} />
        <StatCard title="Total Seats"       value={s.totalSeats ?? 0}       icon={Armchair}      color="text-blue-600"   bgColor="bg-blue-50"   subtitle={`${s.availableSeats ?? 0} available`} />
        <StatCard title="Today's Attendance" value={s.todayAttendance ?? 0} icon={CalendarCheck} color="text-purple-600" bgColor="bg-purple-50" subtitle="Present today" />
      </div>

      {/* Stats Row 2 */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard title="Month Revenue"  value={`₹${(s.monthlyRevenue ?? 0).toLocaleString('en-IN')}`}  icon={IndianRupee}  color="text-emerald-600" bgColor="bg-emerald-50" subtitle={`${s.paymentsThisMonth ?? 0} payments`} />
        <StatCard title="Month Expense"  value={`₹${(s.monthlyExpense ?? 0).toLocaleString('en-IN')}`}  icon={TrendingDown} color="text-orange-600"  bgColor="bg-orange-50" />
        <StatCard title="Net Profit"     value={`₹${(s.netProfit ?? 0).toLocaleString('en-IN')}`}       icon={TrendingUp}   color={(s.netProfit ?? 0) >= 0 ? 'text-green-600' : 'text-red-600'} bgColor={(s.netProfit ?? 0) >= 0 ? 'bg-green-50' : 'bg-red-50'} subtitle="Revenue - Expenses" />
        <StatCard title="Occupied Seats" value={s.occupiedSeats ?? 0}                                    icon={Zap}          color="text-yellow-600" bgColor="bg-yellow-50" subtitle={`of ${s.totalSeats ?? 0} total`} />
      </div>

      {/* Chart + Subscription */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        <div className="lg:col-span-2 bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100">
          <h2 className="font-semibold text-gray-900 mb-4 text-sm sm:text-base">Revenue vs Expenses (Last 6 Months)</h2>
          {chartData.length === 0 ? (
        ...
        <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 space-y-4">
          ...
          <div className="space-y-2.5">
            {[
              ...
              ['Seats Allowed', <span className="font-medium text-sm">{library?.totalSeatsAllowed ?? 0}</span>],
              ['Expiry',        <span className="font-medium text-xs">{library?.subscriptionExpiry ? new Date(library.subscriptionExpiry).toLocaleDateString('en-IN') : '—'}</span>],
              ['Days Left',     <span className={`font-bold text-sm ${daysLeft <= 15 ? 'text-red-600' : 'text-green-600'}`}>{daysLeft} days</span>],
            ].map(([label, val]) => (
          ...
          <div>
            ...
            <div className="w-full bg-gray-100 rounded-full h-2">
              <div
                ...
                style={{ width: `${Math.min(100, ((s.occupiedSeats ?? 0) / (library?.totalSeatsAllowed || 1)) * 100)}%` }}
              />
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
          <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-2">
            ...
            <h2 className="font-semibold text-gray-900 text-sm">Expiring This Week ({expiringStudents.length})</h2>
          </div>
          <div className="divide-y divide-gray-50">
            {expiringStudents.length === 0 ? (
              <p className="text-center py-8 text-gray-400 text-sm">No students expiring soon 🎉</p>
            ) : expiringStudents.slice(0, 6).map(st => (
              <div key={st._id} className="px-4 py-3 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-gray-400 truncate">Seat {st.seatNo || '—'} • {st.mobile}</p>
                  </div>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-xs text-red-500 font-medium">
                    {st.expiryDate ? new Date(st.expiryDate).toLocaleDateString('en-IN') : '—'}
                  </p>
                  <p className="text-xs text-gray-400">₹{st.monthlyFee}/mo</p>
                </div>
              </div>
            ))}
          </div>
